...
from bambooai.messages import prompts, reg_ex, tools_definition
...
class BambooAI:
    def __init__(self, df: pd.DataFrame = None,
        ...
        code_gen_template_dict = {name: getattr(prompts, name) for name in code_gen_templates}
    ...
    def format_image_message(self, agent, message_content, image, provider, model):
        ...
        if agent == 'Code Generator' or agent == 'Planner':
            ...
            message_content = prompts.plot_query.format(message_content)
    ...
    def prompts(self) -> Prompts:
        # Retrieve the prompts service from the service container
        return services.get_prompts()

    def attach_prompts(self, prompts: Prompts):
        # Interface for the user to attach the prompts to the BambooAI instance
        # Use setter to set the prompts
        services.register_prompts(prompts)
    ...
    def taskmaster(self, question, df_columns, aux_datasets_columns, image=None):
        ...
        ######## Select Expert ###########
        if image:
            template = prompts.plot_query_routing.format(question)
        else:
            template = prompts.expert_selector_user.format(question)
        ...
        if expert == 'Data Analyst':
            self.select_analyst_messages.append({"role": "user", "content": prompts.analyst_selector_user.format(self.format_tasks(self.tasks),
            ...
            if analyst == 'Data Analyst DF':
                example_plan = prompts.default_example_plan_df
            elif analyst == 'Data Analyst Generic':
                example_plan = prompts.default_example_plan_gen
            ...
            if analyst == 'Data Analyst DF':
                ...
                if models.get_model_name("Planner")[0] not in self.reasoning_models:
                    self.eval_messages.append({"role": "user", "content": prompts.planner_user_df.format(utils.get_readable_date(), 
                    ...
                    self.eval_messages.append({"role": "user", "content": prompts.planner_user_df_reasoning.format(utils.get_readable_date(), 
                                                                                                                self.format_qa_pairs(self.qa_pairs), 
                                                                                                                intent_breakdown, 
                                                                                                                None if self.df_id is None else dataframe_head,
                                                                                                                utils.aux_datasets_to_string(file_paths=self.auxiliary_datasets,execution_mode=self.execution_mode,executor_client=self.api_client),
                ...
                self.code_messages[0] = {"role": "system", "content": prompts.code_generator_system_df}
                
            elif analyst == 'Data Analyst Generic':
                if models.get_model_name("Planner")[0] not in self.reasoning_models:
                    self.eval_messages.append({"role": "user", "content": prompts.planner_user_gen.format(utils.get_readable_date(), 
                                                                                                       self.format_qa_pairs(self.qa_pairs), 
                                                                                                       intent_breakdown, example_plan)
                                                                                                       })
                else:
                    self.eval_messages.append({"role": "user", "content": prompts.planner_user_gen_reasoning.format(utils.get_readable_date(), 
                                                                                                                 self.format_qa_pairs(self.qa_pairs), 
                                                                                                                 intent_breakdown)
                                                                                                                 })
                ...
                # Replace first dict in messages with a new system task. This is to distinguish between the two types of analysts
                self.code_messages[0] = {"role": "system", "content": prompts.code_generator_system_gen}
            agent = 'Planner'

        elif expert == 'Research Specialist':
            self.eval_messages.append({"role": "user", "content": prompts.theorist_system.format(utils.get_readable_date(),
                                                                                              None if self.df_id is None else df_columns,
                                                                                              aux_datasets_columns, 
    ...
    def _process_question(self, question, image, user_code):
        ...
        if user_code is None:
            ...
            if analyst == 'Data Analyst DF':
                example_code = prompts.default_example_output_df
            else:
                example_code = prompts.default_example_output_gen
    ...
    def correct_code_errors(self, error, error_corrections, code_messages, analyst, code_type, code=None):
        ...
        if code_type == 'user' and error_corrections == 1:
            if self.model_dict[model]['templ_formating'] == 'xml':
                code_messages.append({"role": "user", "content": prompts.error_corector_edited_system.format(code, error, self.python_version, self.pandas_version, self.plotly_version)})
            else: # templ_formating is 'text'
                code_messages.append({"role": "user", "content": prompts.error_corector_edited_system_reasoning.format(code, error, self.python_version, self.pandas_version, self.plotly_version)})
        else:
            if self.model_dict[model]['templ_formating'] == 'xml':
                code_messages.append({"role": "user", "content": prompts.error_corector_system.format(error, self.python_version, self.pandas_version, self.plotly_version)})
            else: # templ_formating is 'text'
                code_messages.append({"role": "user", "content": prompts.error_corector_system_reasoning.format(error, self.python_version, self.pandas_version, self.plotly_version)})
    ...
    def review_plan(self,code, plan):
        ...
        self.plan_review_messages = [{"role": "user", "content": prompts.reviewer_system.format(code, plan)}]
    ...
    def summarise_solution(self, intent_breakdown, plan, results, code=None):
        ...
        # Initialize the messages list with a user message containing the task prompt
        if code is not None:
            self.insight_messages = [{"role": "user", "content": prompts.solution_summarizer_custom_code_system.format(code, results)}]
        else:
            self.insight_messages = [{"role": "user", "content": prompts.solution_summarizer_system.format(intent_breakdown, plan, results)}]
